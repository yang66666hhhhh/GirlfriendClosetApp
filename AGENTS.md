...
│   │   ├── Settings/          # ImageMaintenanceSettingsPanel, WeatherPreferencesSettingsPanel, BackupSettingsPanel 等设置页稳定区块
...
| SettingsTab | 主题切换、天气、备份、图片维护；图片治理区由 `ImageMaintenanceSettingsPanel` 承接，天气推荐偏好区由 `WeatherPreferencesSettingsPanel` 承接，备份恢复区由 `BackupSettingsPanel` 承接 | 无（使用 ViewModel） |
