...
- 顶部推荐区采用“左主右辅”结构：左侧主推荐卡承接天气、标题、状态与主操作；右侧候选区不再保留独立外框和数量标题，而是直接挂两张候选卡，整体高度绑定左侧主推荐卡真实高度
- 右侧候选卡内部固定为“预览区 + 内容区”双列：预览区纵向吃满卡片高度，内容区使用候选编号胶囊、最多两枚理由标签、穿着状态标签、一行摘要和底部操作按钮，主题色通过辅助色 token 动态适配 Rose / Blue 主题
...
- 使用 `候选 1` / `候选 2` 作为主胶囊标签，不再使用解释性提示文案
- 胶囊标签分别复用辅助色 token，避免写死颜色，同时保持不同候选的辨识度
- 候选理由最多展示两枚标签，避免单卡因说明过长被撑高
- 左侧主推荐卡和右侧候选 rail 通过真实高度绑定保持上下边缘对齐
