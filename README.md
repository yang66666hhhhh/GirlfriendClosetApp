...
- 推荐调试：点击推荐搭配的"详情"按钮，查看完整评分分解（季节、收藏、穿着、场景、偏好等维度）；推荐卡片和详情弹窗会额外展示推荐理由、亮点标签、注意事项和与下一候选的对比说明
...
- 本地数据治理：
  ...
  - 导出前校验、图片覆盖统计与警告提示
  - 导入结果摘要、缺失图片提示、备份历史
  - 导入失败时显示失败阶段、数据库是否已回滚，以及过程中已恢复的图片数量
...
- `TagSectionPanel` 复用风格标签区和场景标签区的外层布局
...
- 衣柜瀑布流当前采用“分页 + 分段渲染 + 接近底部自动预扩容”的折中策略，减少 `MasonryPanel` 在大量衣物下的首屏和滚动布局压力，同时保留“加载更多”作为显式兜底入口
...
`ClosetApp.UI/Components/Settings/AppearanceSettingsPanel` 是设置页外观面板，集中处理主题切换、版本展示和应用目录入口。
...
- `PremiumClothingCard` 只在进入可见区后发起异步加载，并在滚动复用或卸载时取消旧请求
- `ClothingImageLoader` 会合并同一图片的并发加载和尺寸探测请求，避免重复后台解码
- `ClothingImageLoader` 使用弱引用内存缓存，并对解码失败结果做短时负缓存，避免坏图在快速滚动时被反复尝试加载
