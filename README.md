...
这些图片治理操作集中在 `ImageMaintenanceSettingsPanel`，天气与推荐偏好集中在 `WeatherPreferencesSettingsPanel`，外观信息集中在 `AppearanceSettingsPanel`，备份恢复操作集中在 `BackupSettingsPanel`，`SettingsTab` 只负责页面初始化和跨分组刷新协调。
