...
- The log-maintenance block is `LogMaintenanceSettingsPanel`. Keep log stats refresh, open-log-directory, and clear-history-log actions inside that panel; `SettingsTab` should only coordinate page-level refreshes across settings groups.
...
- `Components/Outfit/Controls/OutfitCard` should keep outfit readability first: keep the card as a light browse surface around `OutfitPreviewCanvas`, with only status/favorite/more actions on-card; outfit management and AI image workflows belong in `Components/Shared/Modal/OutfitWorkspaceDialog`.
